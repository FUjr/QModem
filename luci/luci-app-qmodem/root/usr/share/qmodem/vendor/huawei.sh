...
# Copyright (C) 2025 498110811 <ixiushu@qq.com>
#脚本目录
SCRIPT_DIR="/usr/share/modem"
_Vendor="huawei"
_Author="Siriling,498110811"
_Maintainer="498110811 <ixiushu@qq.com>"
...
get_mode()
{
    ...
    local mode_num=$(at ${at_port} ${at_command} | awk -F': ' '{print $2}' | tr -d '\r')
    local mode
    case "$platform" in
        case "$platform" in
        "hisilicon")
...
get_network_prefer_nr()
{
    ...
    local response_res=$(at ${at_port} ${at_command} | grep "\^SYSCFGEX:" | sed 's/\^SYSCFGEX://g')
    local response=$(echo "$response_res" | awk -F'"' '{print $2}');
    network_prefer_3g="0";
    network_prefer_4g="0";
    network_prefer_5g="0";
    ...
    if [ -n "$auto" ]; then
        ...
        local nr=$(echo "${response}" | grep "08")
        if [ -n "$wcdma" ]; then
            network_prefer_3g="1"
        fi
        if [ -n "$lte" ]; then
...
set_network_prefer_nr()
{
    ...
    #设置模组
    at_command='AT^SYSCFGEX="'${network_prefer_config}'",40000000,1,2,7FFFFFFFFFFFFFFF,,'
    at "${at_port}" "${at_command}"
}
...
get_voltage()
{
    at_command="AT+ADCREADEX=0"
	local voltage=$(at ${at_port} ${at_command}| grep "+ADCREADEX:" | awk -F' ' '{print $2}' | sed 's/\r//g')
    [ -n "$voltage" ] && {
...
{   
    ...
    local temp
    local line=1
    response=$(at ${at_port} ${at_command} | grep "\^CHIPTEMP" | awk -F',' '{print $6}' | sed 's/\r//g')
    if [ -n "$response" ]; then
...
sim_info()
{
    m_debug  "Huawei sim info"
    
    #SIM Slot（SIM卡卡槽）
    at_command="AT+QUIMSLOT?"
	sim_slot=$(at $at_port $at_command | grep "\^SIMSLOT:" | awk -F': ' '{print $2}' | awk -F',' '{print $2}')
...
network_info()
{
    ...
    #最大比特率，信道质量指示
    at_command='AT^DHCP?'
    response=$(at $at_port $at_command | grep "\^DHCP:" | sed 's/\^DHCP: //g' | sed 's/\r//g')
    # CQL UL（上行信道质量指示）
    cqi_ul=$(echo "$context" | awk -F',' '{print $8}')
    #CQI DL（下行信道质量指示）
    cqi_dl=$(echo "$context" | awk -F',' '{print $7}')
    [ -z "$cqi_ul" ] && {
        at_command="AT^DHCPV6?"
        response=$(at $at_port $at_command | grep "\^DHCPV6:" | sed 's/\^DHCPV6: //g' | sed 's/\r//g')
        cqi_ul=$(echo "$response" | awk -F',' '{print $8}')
        cqi_dl=$(echo "$response" | awk -F',' '{print $7}')
    }
   #AMBR UL（上行签约速率，单位，Mbps）
    ambr_ul=$(awk "BEGIN{ printf \"%.2f\", $cqi_ul / 1000000 }" | sed 's/\.*0*$//')
    #AMBR DL（下行签约速率，单位，Mbps）
    ambr_dl=$(awk "BEGIN{ printf \"%.2f\", $cqi_dl / 1000000 }" | sed 's/\.*0*$//')
